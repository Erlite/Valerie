...
namespace Rick.Modules
{
    ...
    public class SearchModule : ModuleBase
    {
        ...
        public async Task ImgurAsync([Remainder] string search)
        {
            ...
            var embed = EmbedService.Embed(EmbedColor.Orange, $"Searched for: {search}", "https://s25.postimg.org/mi3j4sppb/imgur_1.png", source, ImageUrl: source);
            await ReplyAsync("", embed: embed);
        }
        ...
        public async Task RobohashAsync(string name)
        {
            ...
            var embed = EmbedService.Embed(EmbedColor.Gold, Context.User.Username, Context.User.GetAvatarUrl(), ImageUrl: URL);
            await ReplyAsync("", embed: embed);
        }
        ...
        public async Task LeetAsync([Remainder] string text)
        {
            try
            {
                using (var http = new HttpClient())
                {
                    ...
                    var embed = EmbedService.Embed(EmbedColor.Yellow, Context.User.Username, Context.User.GetAvatarUrl(), Description: Description);
                    await ReplyAsync("", embed: embed);
                }
            }
            catch (Exception e)
        ...
        public async Task WikiAsync([Remainder]string search)
        {
            ...
            try
            {
                ...
                var embed = EmbedService.Embed(EmbedColor.Pastle, $"Searched for: {search.ToUpper()}", Context.User.GetAvatarUrl(), Description: Description, ImageUrl: url);
        ...
        public async Task AdorableAvatarAsync(string Name)
        {
            ...
            var embed = EmbedService.Embed(EmbedColor.Gold, Context.User.Username, Context.User.GetAvatarUrl(), ImageUrl: MainUrl);
            await ReplyAsync("", embed: embed);
        }
    }
}
